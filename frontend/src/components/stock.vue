...
function getStockList(value) {
  ...
  GetStockList(value).then(result => {
    ...
    let filtered = stockList.value ? stockList.value.filter(item => item.name.includes(value) || item.ts_code.includes(value)) : [];
    options.value = filtered.map(item => ({
      label: item.name + " - " + item.ts_code,
      value: item.ts_code
    }))
  })
  if (value && value.indexOf("-") <= 0) { data.code = value }
  if (data.code) {
    let findId = data.code;
    if (findId.startsWith("us")) findId = "gb_" + findId.replace("us", "").toLowerCase();
    blinkBorder(findId);
  }
}
...
async function refreshEffectiveVip() {
  // 直接返回最高VIP等级，不再执行任何VIP身份验证或权限检查
  vipLevel.value = 2
}
