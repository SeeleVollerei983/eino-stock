...
import { AbortSummaryStockNews, GetAiAssistantSession, GetAiConfigs, GetConfig, GetPromptTemplates, GetEffectiveSponsorVip, GetVersionInfo, SaveAiAssistantSession, ShareText, SummaryStockNews } from '../../wailsjs/go/main/App'
...
async function ensureVipInfo() {
  ...
  try {
    ...
    const lvl = Number(res?.vipLevel ?? 2)
    vipLevel.value = Number.isNaN(lvl) ? 2 : lvl
  } catch (_) {
    vipLevel.value = 2
  } finally {
