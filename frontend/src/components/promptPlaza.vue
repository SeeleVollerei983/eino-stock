...
import {GetConfig, GetMachineId, CheckDeviceBinding, QuitApp, GetEffectiveSponsorVip, AddPromptTemplate} from "../../wailsjs/go/main/App";
...
async function syncVipInfo() {
  if (!token.value) return
  try {
    const sponsorInfo = await GetEffectiveSponsorVip()
    const vipLevel = sponsorInfo?.vipLevel ? Number(sponsorInfo.vipLevel) : 2
